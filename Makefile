...
build: #Собрать проект(composer install + npm install + yarn encore dev + dump sql + update db)
	docker-compose exec php composer install && docker-compose exec php npm install && docker-compose exec php npm rebuild node-sass && docker-compose exec php php bin/console doctrine:schema:update --dump-sql && docker-compose exec php php bin/console doctrine:schema:update --force && docker-compose exec php yarn install && docker-compose exec php yarn encore dev

build-all: #Собрать проект(composer install + npm install + yarn encore dev + dump sql + update db)
	docker-compose exec php composer install && docker-compose exec php npm install && docker-compose exec php npm rebuild node-sass && docker-compose exec php php bin/console doctrine:schema:update --dump-sql && docker-compose exec php php bin/console doctrine:schema:update --force && docker-compose exec php yarn install && docker-compose exec php yarn encore dev && docker-compose exec php php bin/console doctrine:fixtures:load -n
...
test-all: #Запуск всех тестов
	docker-compose exec php php bin/console doctrine:schema:update --dump-sql --env=test && docker-compose exec php php bin/console doctrine:schema:update --force --env=test && docker-compose exec php php bin/console doctrine:fixtures:load --env=test -n && docker-compose exec php sh tests/reset-password-test.sh && docker-compose exec php php vendor/bin/codecept run --steps --env=test
